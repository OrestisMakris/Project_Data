...
float_ocean_data Interpolation_Search(float_ocean_data *ocean_data , float_ocean_data target  , int n )
{
    int left = 0;
    int right = n-1;
    int next ;
    ...
    while(Compare_Dates(target , ocean_data[next]) != 0 && left < right) {
        next = floor((getDifference_date( ocean_data[left], target) * (right - left))/getDifference_date(ocean_data[left] , ocean_data[right])) + left;
...
int main()
{
    ...
    clock_t t;
    t = clock();
    temp =Interpolation_Search(ocean_data ,temp , i);
    t = clock() - t;
